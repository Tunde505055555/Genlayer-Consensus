...
class ConsensusOracle(gl.Contract):
    ...
    @gl.public.view
    def get_submission_status(self, qid: int) -> dict:
        questions = self._questions()
        if qid < 0 or qid >= len(questions):
            return {"id": qid, "status": "not_found", "completed": False, "verdict": None}
        q_data = questions[qid]
        completed = q_data.get("status") == STATUS_REACHED and q_data.get("verdict") is not None
        return {
            "id": qid,
    ...
    @gl.public.write
    def submit_question(self, question: str) -> int:
        q = question.strip()
        if len(q) < 4:
...
"""
            ...
            # Restrict reported source URLs strictly to the pages actually fetched/retrieved
            ...
            if isinstance(raw_sources, list):
                for item in raw_sources:
                    link = str(item).strip()[:300]
                    if link in fetched_set and link not in sources_out:
                        sources_out.append(link)
                        if len(sources_out) >= 5:
                            break
            if not sources_out and fetched_urls:
                sources_out = fetched_urls[:5]

            # Derive evidence status and source count directly from pages actually retrieved
            evidence_status = fetch_status
            sources_count = domain_count
        ...
        principle = (
            "Two oracle verdicts are equivalent when: "
            "1. Their answer fields make the same substantive judgment about the question: "
            "the same direction (yes/no/true/false/undetermined) and no contradiction in any key "
            "figure, date or entity that both state. They are NOT equivalent if one asserts a definite "
            "verdict while the other says the question is undetermined or unsupported by evidence, "
            "or if they state contradictory facts. "
            "2. Their evidence-bearing verdict fields agree: 'evidence_status' must match (both must agree "
            "whether evidence was found, insufficient, or search failed; a verdict with 'evidence_found' "
            "is NOT equivalent to 'insufficient_evidence' or 'search_failed'). "
            "3. Their 'sources_count' must be consistent with the consensus evidence status (neither can "
            "claim consensus if one found zero sources while the other claims multiple independent sources, "
            "or if their independent source counts disagree on whether the minimum threshold was met). "
            "Because validators fetch live web pages independently, differences in wording, explanation length, "
            "confidence numbers within the valid range, principle text, per-claim phrasing, and which specific "
            "retrieved URLs each validator cited are expected and must NOT be treated as a disagreement."
        )
